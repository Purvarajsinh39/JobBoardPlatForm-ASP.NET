...
public class AdminController : BaseController
{
    ...
    {
        List<JobWithEmployerViewModel> jobs = new List<JobWithEmployerViewModel>();
        string connectionString = ConfigurationManager.ConnectionStrings["JobBoardDB"].ConnectionString;


        using (SqlConnection con = new SqlConnection(connectionString))
        {
            string query = @"
            SELECT J.Id, J.Title, J.Description, J.Category, J.Location, J.PostedDate, J.IsApproved,
        ...
        ";
            ...
            while (rdr.Read())
            {
                jobs.Add(new JobWithEmployerViewModel
                {
                    ...
                    EmployerName = rdr["EmployerName"].ToString()
                });
            }
        }
    ...
    public ActionResult Notifications()
    {
        ...
        return View(notifications);  // Views/Admin/Notifications.cshtml (use the seeker's template or create similar)
    }
    ...
    // Quiz Stats (new, for graph)
    // Quiz Stats (updated with debug)
    public ActionResult QuizStats()
    {
        var model = new QuizStatsViewModel();
        ...
        using (SqlConnection con = new SqlConnection(conStr))
        {
            ...
            // Total Attempts
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM QuizAttempts", con))
                model.TotalAttempts = (int)cmd.ExecuteScalar();

            // Passed
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM QuizAttempts WHERE Passed = 1", con))
                model.Passed = (int)cmd.ExecuteScalar();

            // Failed
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM QuizAttempts WHERE Passed = 0", con))
                model.Failed = (int)cmd.ExecuteScalar();
        }

        // Debug: Pass counts to view
        ViewBag.DebugMsg = $"Total Attempts: {model.TotalAttempts}, Passed: {model.Passed}, Failed: {model.Failed}";

        return View(model);  // Views/Admin/QuizStats.cshtml
    }
